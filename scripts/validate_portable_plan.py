...
def validate_defaults(data: dict[str, Any]) -> list[str]:
    """Validate TimeBudget reusable defaults 1.0.0."""

    errors: list[str] = []
    _validate_global_strings(data, "$", errors)
    _expect_exact_keys(data, DEFAULTS_KEYS, "$", errors)
    if data.get("format") != "timebudget-defaults":
        errors.append("format must equal timebudget-defaults")
    if data.get("schema_version") != "1.0.0":
        errors.append("schema_version is unsupported; supported versions: 1.0.0")

    zone_name = data.get("timezone")
    if zone_name is not None:
        _expect_text(zone_name, "timezone", errors, 100)
        if isinstance(zone_name, str):
            try:
                ZoneInfo(zone_name)
            except ZoneInfoNotFoundError:
                errors.append("timezone is not a recognized IANA timezone")

    for key in ("planning_start", "planning_end"):
        value = data.get(key)
        if not isinstance(value, str) or not TIME_RE.fullmatch(value):
            errors.append(f"{key} must be HH:MM in 24-hour time")

    buffer_value = _expect_object(data.get("buffer"), "buffer", errors)
    if buffer_value is not None:
        _expect_exact_keys(buffer_value, BUFFER_KEYS, "buffer", errors)
        mode = buffer_value.get("mode")
        _expect_enum(mode, {"recommended", "fixed"}, "buffer.mode", errors)
        minutes = buffer_value.get("minutes")
        if mode == "recommended":
            if minutes is not None:
                errors.append("buffer.minutes must be null in recommended mode")
        elif mode == "fixed":
            _expect_int(minutes, "buffer.minutes", errors, maximum=MAX_DURATION)
    ...
    _expect_enum(
        data.get("default_task_priority"),
        {"must", "should", "could"},
        "default_task_priority",
        errors,
    )
...
def recalculate_snapshot(
    data: dict[str, Any], as_of: datetime
) -> dict[str, Any] | None:
    """Recalculate convenience fields at a reliable, timezone-aware current time."""
...
def expected_snapshot(data: dict[str, Any]) -> dict[str, Any] | None:
    """Recalculate the convenience snapshot at its supplied ``as_of`` time."""

    supplied = data.get("snapshot")
    if not isinstance(supplied, dict) or not isinstance(supplied.get("as_of"), str):
        return None
    try:
        as_of = datetime.fromisoformat(supplied["as_of"].replace("Z", "+00:00"))
    except ValueError:
        return None
    return recalculate_snapshot(data, as_of)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--defaults", action="store_true", help="Validate a defaults artifact")
    parser.add_argument("artifact", type=Path, help="Path to a portable JSON artifact")
    ...
    try:
        ...
        return 1
    errors = validate_defaults(data) if args.defaults else validate_plan(data)
    if errors:
        for error in errors:
    ...
    kind = "defaults" if args.defaults else "portable plan"
    print(f"VALID: TimeBudget {kind} 1.0.0")
    return 0
