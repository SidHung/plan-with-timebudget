...
SCHEMA_PATH = ROOT / "references" / "portable-plan.schema.json"
...
class TimeBudgetContractTests(unittest.TestCase):
    ...
    def test_defaults_schema_is_closed_and_fixture_is_valid(self):
        schema = json.loads(DEFAULTS_SCHEMA_PATH.read_text(encoding="utf-8"))
        defaults = json.loads(DEFAULTS_FIXTURE_PATH.read_text(encoding="utf-8"))
        self.assertFalse(schema["additionalProperties"])
        self.assertFalse(schema["properties"]["buffer"]["additionalProperties"])
        self.assertEqual([], validator.validate_defaults(defaults))

    def test_defaults_reject_unknown_fields_and_mode_contradictions(self):
        defaults = json.loads(DEFAULTS_FIXTURE_PATH.read_text(encoding="utf-8"))
        defaults["unknown"] = True
        self.assertTrue(any("unsupported" in error for error in validator.validate_defaults(defaults)))
        defaults.pop("unknown")
        defaults["buffer"]["minutes"] = 30
        self.assertTrue(any("must be null" in error for error in validator.validate_defaults(defaults)))

    def test_defaults_allow_omitting_saved_timezone_with_null(self):
        defaults = json.loads(DEFAULTS_FIXTURE_PATH.read_text(encoding="utf-8"))
        defaults["timezone"] = None
        self.assertEqual([], validator.validate_defaults(defaults))
